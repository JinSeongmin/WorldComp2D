...
def gaussian_heatmap(label_in_hm, size=27, sigma=1.5):
    y = label_in_hm[..., 0]
    x = label_in_hm[..., 1]
    
    valid_mask = ((y>=0) & (y<=size-1) & (x>=0) & (x<=size-1))
    
    ys = torch.arange(size).to(device).view(1,1,size,1)
    xs = torch.arange(size).to(device).view(1,1,1,size)
    ...
    heatmap = heatmap * valid_mask[..., None, None].float()
    
    return heatmap, valid_mask



def save_model(task, aux_loc_1, aux_loc_2, aux_loc_3, loss_hist, optimizer, epoch):
    state = {
        'aux_loc_1' : aux_loc_1.state_dict(),
        'aux_loc_2' : aux_loc_2.state_dict(),
        'aux_loc_3' : aux_loc_3.state_dict(),
        'opt': optimizer.state_dict(),
        'loss_hist' : loss_hist,
        'epoch': epoch}
    path = "../../Checkpoint/"
    
    torch.save(state, path+'{}_Auxiliary_localizers_{}epoch.pth'.format(task, epoch))
