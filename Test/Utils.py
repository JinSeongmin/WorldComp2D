...
def load_proximity_dependent_encoder(task, enc_1, enc_2, enc_3) : 
    ...
    state = torch.load(path+"{}_Proximity_dependent_encoders.pth".format(task), map_location=device)
    
    enc_1.load_state_dict(state['enc_1'])
    enc_2.load_state_dict(state['enc_2'])
    enc_3.load_state_dict(state['enc_3'])
...
def load_localizer(task, loc_1, loc_2, loc_3) : 
    path = "../Pretrained_modules/"
    state = torch.load(path+"{}_Localizers.pth".format(task), map_location=device)
    
    loc_1.load_state_dict(state['loc_1'])
    loc_2.load_state_dict(state['loc_2'])
    loc_3.load_state_dict(state['loc_3'])
...
def load_auxiliary_localizer(task, aux_loc_1, aux_loc_2, aux_loc_3) : 
    path = "../Pretrained_modules/"
    state = torch.load(path+"{}_Auxiliary_localizers.pth".format(task), map_location=device)
    
    aux_loc_1.load_state_dict(state['aux_loc_1'])
    aux_loc_2.load_state_dict(state['aux_loc_2'])
    aux_loc_3.load_state_dict(state['aux_loc_3'])
