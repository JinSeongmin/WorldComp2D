...
def load_proximity_dependent_encoder(task, enc_1, enc_2, enc_3) : 
    path = "../Pretrained_modules/"
    state = torch.load(path+"Proximity_dependent_encoders.pth", map_location=device)
    
    enc_1.load_state_dict(state['{}_enc_1'.format(task)])
    enc_2.load_state_dict(state['{}_enc_2'.format(task)])
    enc_3.load_state_dict(state['{}_enc_3'.format(task)])
    
    return enc_1, enc_2, enc_3



def load_landmark_coordinate_prior(task) : 
    path = "../Pretrained_modules/"
    state = torch.load(path+"Landmark_coordinate_priors.pth", map_location=device)
    
    coords_prior = state['{}_coords_prior'.format(task)]
    
    return coords_prior



def load_localizer(task, loc_1, loc_2, loc_3) : 
    path = "../Pretrained_modules/"
    state = torch.load(path+"Localizers.pth", map_location=device)
    
    loc_1.load_state_dict(state['{}_loc_1'.format(task)])
    loc_2.load_state_dict(state['{}_loc_2'.format(task)])
    loc_3.load_state_dict(state['{}_loc_3'.format(task)])
    
    return loc_1, loc_2, loc_3



def load_auxiliary_localizer(task, aux_loc_1, aux_loc_2, aux_loc_3) : 
    path = "../Pretrained_modules/"
    state = torch.load(path+"Auxiliary_localizers.pth", map_location=device)
    
    aux_loc_1.load_state_dict(state['{}_aux_loc_1'.format(task)])
    aux_loc_2.load_state_dict(state['{}_aux_loc_2'.format(task)])
    aux_loc_3.load_state_dict(state['{}_aux_loc_3'.format(task)])
    
    return aux_loc_1, aux_loc_2, aux_loc_3
...
def extract_observation(task, imgs, x, using_2nd_scale=True) : 
    view_size = [27, 27]
    scale_ratio = [1, 4]
    view_size_2 = [view_size[0]*scale_ratio[1], view_size[0]*scale_ratio[1]]
    ...
    if using_2nd_scale : 
        o = torch.cat((extract_shifted_patches(task, imgs, need_pad, x.reshape(-1,2), 27),
                       F_.resize(extract_shifted_patches(task, imgs, need_pad, x.reshape(-1,2), 27*4+1), view_size)), dim=1).view(-1, 2*channel, 27, 27)
    else : 
        o = extract_shifted_patches(task, imgs, need_pad, x.reshape(-1,2), 27).view(-1, channel, 27, 27)    
    return o
...
def NME_calc(task, inferred_coord, pts, center, scale) :
    B = inferred_coord.size(0)
    n_l = inferred_coord.size(1)
    pts = pts.view(-1, n_l, 2)
    
    inferred_coord = inferred_coord.cpu().numpy()
    
    if task == 'AFLW' : 
        norm_factor = torch.zeros(B)
        
        for b in range(B) :    
            ul = np.array(transform_pixel([0, 0], center[b], scale[b], [256, 256], invert=1), dtype=np.float32)
            br = np.array(transform_pixel([256, 256], center[b], scale[b], [256, 256], invert=1), dtype=np.float32)
            norm_factor[b] = ((br-ul)[0]**2 + (br-ul)[1]**2)**0.5
            
            for l in range(n_l) : 
                inferred_coord[b, l] = transform_pixel(inferred_coord[b, l]+1, center[b], scale[b], [256,256], invert=1, rot=0)
    
    else : 
        norm_factor = torch.norm(pts[:, 9] - pts[:, 8], dim=-1) if task == 'COFW' else torch.norm(pts[:, 45] - pts[:, 36], dim=-1)
        
        for b in range(B) :     
            for l in range(n_l) : 
                inferred_coord[b, l] = transform_pixel(inferred_coord[b, l]+1, center[b], scale[b], [256,256], invert=1, rot=0)
    
    inferred_coord = inferred_coord.reshape(-1, n_l, 2)
    error = torch.norm(pts - inferred_coord, dim=-1).mean(-1)
    
    NME = error / norm_factor
    return NME
